...
class IpnResponder
{
    ...
    public function getItemsPurchased($ipnMessage)
    {
        ...
        $items = array_filter(array_map($findItemKeys, array_keys($ipnMessage), array_values($ipnMessage)));
    ...
    private function ipnMessageIsNotFromPaypal($ipnMessage)
    {
        if (! $this->isVerified($ipnMessage)) {
            ...
            Log::info(__METHOD__.":".__LINE__.": The IPN message couldn't be verified {$this->get('txn_id', $ipnMessage)}.");
    ...
    private function ipnMessageHasBeenReceivedBefore($ipnMessage)
    {
        if ($this->hasBeenReceivedBefore($ipnMessage)) {
            ...
            Log::info(__METHOD__.":".__LINE__.": The IPN message has been received before {$this->get('txn_id', $ipnMessage)}.");
