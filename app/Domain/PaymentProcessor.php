...
class PaymentProcessor
{
    ...
    public function process($ipnMessage)
    {
        ...
        $this->orderFullFiller->fulfill($this->project->find($this->getItemsPurchased($ipnMessage)), $ipnMessage->getBuyersEmailAddress());
    ...
    private function ipnMessageIsVerifiedButNoItemsWerePurchased($ipnMessage)
    {
        if (empty($this->getItemsPurchased($ipnMessage))) {
            $this->log(__METHOD__.":".__LINE__.":"."The IPN message was received successfully, but no items were purchased {$ipnMessage->get('txn_id')}.");
