...
class GuideTest extends TestCase {
  ...
  public function testFindOneProjectOneGuide() {
    ...
    $s3->shouldReceive('getObjectUrl')->with(
      ...
      'technique201707/guides/technique201707.pdf', config('flowerbug.s3.signed_url_expiration'))
      ->once();
  ...
  public function testFindOneProjectThreeGuides() {
    ...
    $s3->shouldReceive('getObjectUrl')->with(
      config('flowerbug.s3.projects_bucket'),
      'technique201707/guides/technique201707.pdf',
      config('flowerbug.s3.signed_url_expiration')
    )->once();
    $s3->shouldReceive('getObjectUrl')->with(
      config('flowerbug.s3.projects_bucket'),
      'technique201707/guides/technique201707.doc',
      config('flowerbug.s3.signed_url_expiration')
    )->once();
    $s3->shouldReceive('getObjectUrl')->with(
      config('flowerbug.s3.projects_bucket'),
      'technique201707/guides/technique201707.jpg',
      config('flowerbug.s3.signed_url_expiration')
    )->once();
    $guide = new \App\Domain\Guide($s3);
