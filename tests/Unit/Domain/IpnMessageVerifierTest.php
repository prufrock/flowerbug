...
class IpnMessageVerifierTest extends TestCase
{
    ...
    {
        $this->assertInstanceOf(\App\Domain\IpnMessageVerifier::class, $this->app->make(\App\Domain\IpnMessageVerifier::class));
    }
    ...
    public function testComputeExists()
    {
        ...
        $this->assertTrue((new \ReflectionObject($verifier))->hasMethod('compute'), "IpnMessageVerifier doesn't have a compute method.");
    }
    ...
    public function testHasCreateMethod()
    {
        ...
        $this->assertTrue((new \ReflectionObject($verifier))->hasMethod('create'), "IpnMessageVerifier doesn't have a create method.");
    }
    ...
    public function testVerifyAValidMessage()
    {
        ...
        $fproxy->shouldReceive('fsockopen')->with($validationUrl, $validationPort, $errno, $errstr, $validationTimeout)->andReturn(true)->once();
        $fproxy->shouldReceive('fputs')->with(true, $header)->once();
        $response = tmpfile();
    ...
    public function testComputeIsUnableToGetAFileHandle()
    {
        ...
        $fproxy = m::mock('\App\Domain\FilePointerProxy');
        $fproxy->shouldReceive('fsockopen')->with(config('flowerbug.paypal.ipn_verify_url'), $validationPort = config('flowerbug.paypal.ipn_verify_port'), $errno, $errstr, 30)->andReturn(false)->once();
